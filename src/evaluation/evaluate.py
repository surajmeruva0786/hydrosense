...
def main() -> None:
    ...
    run_dir = (
        ...
        if checkpoint_path.parent.name != f"fold_{0}"
    ...
    logger.info(
        "Evaluating %s (%s / %s) using checkpoint %s",
        config["model"],
    ...
    dataset = ManifestDataset(
        ...
        transform=build_default_transform(
            expected_num_frames(int(config["sample_rate"] * config["segment_length"]))
        ),
        split="test",
    )
    if len(dataset) == 0:
        raise RuntimeError(
            f"No test segments found in {manifest_path}. Did preprocessing run with test recordings?"
        )
